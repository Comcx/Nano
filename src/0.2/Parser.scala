...
class Parser {
  ...
  def run (e: SExp, env: List [Bind]): SExp = e match {
    ...
    case L(L(Op("if"), e), L(e0, e1)) => run(e, env) match {
      ...
      case Quote(Var("true")) => run(e0, env)
      case Quote(Var("false")) => run(e1, env)
      case _ => Error("Wrong if function!", L(L(Op("if"), e), L(e0, e1)))
    }


    case L(Op("@"), exp) => run(exp, env) match {
      case Quote(e) => run(e, env)
      case _ => run(exp, env)
    }
    ...
    case L(L(Op("="), x), y) => (run (x, env), run (y, env)) match {
      case (Number (m), Number (n)) => if (m == n) Quote(Var("true"))
                                       else Quote(Var("false"))
      case _ => Error("", L(Str("=?"), L(x, y)))
    }
    case L(Op("'"), e) => Quote(e)
