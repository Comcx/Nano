...
class Reader () {
  ...
  def isNumber(raw: String): Boolean = raw.trim.forall(c => c.isDigit || c == '.') &&
    !isEmpty(raw)
  ...
  def read(raw: String): SExp = {
    if (raw == "") return Error("Nothing at all!", Str("Empty input!"))
    val s = raw.trim.lines.filter(l =>
      if (l.trim != "") l.trim.head != ';' else false).foldLeft("")(_ + _ + "\n").trim
    if (checkParents(s) == false)
      return Error("Parentheses not match! please check!", Str(s.trim))
  ...
  def process(raw: String): SExp = {
    ...
    if (isSingle(s)) { val content = contentInParent(s)
      if (isNumber(content)) {
        /*if (content contains '.') Number(content.toDouble) else */Number(content.toInt)
      }
      ...
      else Error("Weird single?", Str(s))
    }
    else if (isList(s)) {
      val (fst, snd) = split(s)
      L(process(fst), process(snd))
    }
    else Error("Something wrong with input!", Str("sth wrong"))
